      ...
      >
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          ...
          <div className="relative w-full h-64 md:h-80 bg-slate-800 shrink-0">
             ...
             <button 
                onClick={onClose} 
                className="absolute top-6 left-6 p-2 bg-black/20 backdrop-blur-md rounded-full text-white hover:bg-white hover:text-black transition-all"
             >
               <X size={20} />
             </button>
          </div>
                ...
                <div className="flex items-center justify-between mb-6">
                   ...
                   <div className="flex p-1 bg-slate-800 rounded-lg border border-slate-700">
                     <button 
                        ...
                        className={`p-2 rounded-md transition-all ${viewMode === 'scrapbook' ? 'bg-slate-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'}`}
                     ...
                     <button 
                        ...
                        className={`p-2 rounded-md transition-all ${viewMode === 'filmstrip' ? 'bg-slate-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'}`}
                  ...
                  {viewMode === 'scrapbook' ? (
                    ...
                    >
                      {project.images.slice(1).map((img: string, idx: number) => (
                        <div 
                          key={idx}
                          className="break-inside-avoid relative group rounded-lg overflow-hidden border border-slate-800 bg-slate-950 cursor-zoom-in"
                    ...
                    <motion.div 
                      key="filmstrip"
                      initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                      className="flex gap-4 overflow-x-auto pb-4 pt-2 snap-x mandatory custom-scrollbar"
                    >
                      {project.images.slice(1).map((img: string, idx: number) => (
                        <div 
                          key={idx}
                          className="snap-center shrink-0 w-[85%] md:w-[70%] aspect-video relative rounded-lg overflow-hidden border border-slate-800 bg-slate-950 cursor-zoom-in group"
                          onClick={() => setLightboxImg(img)}
                        >
                          <Image 
                            src={img} 
            ...
            <div className="border-t border-slate-800 pt-8">
              ...
              <div className="flex flex-wrap gap-2">
                {project.tech.map((t: string) => (
                  <span key={t} className="px-3 py-1 bg-slate-800 text-slate-300 rounded-md text-xs font-medium border border-slate-700">
                    {t}
                  </span>
                ))}
              </div>
        ...
        <div className="p-6 border-t border-slate-800 bg-slate-900 sticky bottom-0 flex gap-4 shrink-0 z-20">
          <a href="#" className="flex-1 flex items-center justify-center gap-2 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-colors font-bold shadow-lg shadow-blue-900/20">
             Live Demo <ExternalLink size={16} />
          </a>
          <a href="#" className="flex-1 flex items-center justify-center gap-2 py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-lg transition-colors font-bold border border-slate-700">
             Code <Github size={16} />
          </a>
        </div>
      </motion.div>
    ...
    <div className="min-h-screen bg-slate-950 text-slate-300 font-sans selection:bg-blue-500/30 overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-40 bg-slate-950/80 backdrop-blur-md border-b border-slate-800/50">
        ...
        {isMenuOpen && (
          ...
          >
            {["About", "Experience", "Projects", "Skills", "Education", "Contact"].map((item) => (
              <button key={item} onClick={() => scrollTo(item.toLowerCase())} className="text-left py-2 hover:text-blue-400">
                {item}
              </button>
            ))}
          </motion.div>
      ...
      <section id="about" className="relative pt-32 pb-20 px-6 overflow-hidden">
        ...
        <FadeIn className="max-w-4xl mx-auto text-center relative z-10">
          ...
          <p className="text-lg md:text-xl text-slate-400 max-w-2xl mx-auto mb-10 leading-relaxed">
            ...
            Specializing in <b>TypeScript</b> and <b>Full Stack Architecture</b>.
          </p>
          ...
          <div className="mt-16 flex justify-center gap-6 text-slate-500">
            <div className="flex items-center gap-2"><MapPin size={16} /> Manouba, Tunis</div>
            <div className="flex items-center gap-2"><Phone size={16} /> +216 27 004 058</div>
          </div>
        </FadeIn>
      </section>
      ...
      <section id="experience" className="py-20 px-6 bg-slate-900/30">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-12 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-700 before:to-transparent">
            <FadeIn delay={0.1}>
              <TimelineItem index={0} role="Frontend Developer" company="Tunisair" date="02/2024 - 05/2024" stack={["MERN Stack", "React", "Node.js"]}>
                <ul className="list-disc list-inside space-y-2 text-slate-400">
                  <li>Designed and developed a responsive website using the <b>MERN stack</b>.</li>
                  <li>Improved user experience through clean UI/UX design.</li>
                </ul>
              </TimelineItem>
            </FadeIn>
            <FadeIn delay={0.2}>
              <TimelineItem index={1} role="Photo Editor" company="Terkina" date="05/2022 - 08/2025" stack={["Photoshop", "Lightroom"]}>
                <ul className="list-disc list-inside space-y-2 text-slate-400">
                  <li>Retouched <b>2000+ images</b> ensuring brand consistency.</li>
                </ul>
              </TimelineItem>
            </FadeIn>
          </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-col gap-6">
            {projects.map((project) => (
              ...
              >
                <SpotlightCard 
                  className="w-full cursor-pointer hover:border-blue-500/50 transition-colors"
                  ...
                  <div className="p-8">
                    <div className="flex flex-col md:flex-row gap-6 justify-between items-start">
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {project.tech.map((t) => (
                            <span key={t} className="text-xs px-2.5 py-1 bg-slate-950 border border-slate-800 rounded text-slate-400 font-mono">
                              {t}
                            </span>
                          ))}
                        </div>
      ...
      <section id="skills" className="py-20 px-6 bg-slate-900/30">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <SkillCardWrapper title="Languages" icon={<Code className="text-blue-400" />} skills={["Java", "PHP", "JavaScript", "HTML", "CSS"]} />
            <SkillCardWrapper title="Frontend" icon={<Layers className="text-purple-400" />} skills={["React", "Next.js", "Tailwind CSS","Redux"]} />
            <SkillCardWrapper title="Backend" icon={<Database className="text-green-400" />} skills={["Node.js", "Express", "MongoDB", "Spring Boot", "Laravel"]} />
          </div>
        </div>
      </section>
      ...
      <footer id="contact" className="py-20 px-6 border-t border-slate-900 bg-slate-950">
        <FadeIn className="max-w-3xl mx-auto text-center">
          ...
          <div className="flex flex-wrap justify-center gap-6 mb-12">
            <SocialLink href="mailto:maysalimp@gmail.com" icon={<Mail />} label="Email Me" />
            <SocialLink href="https://www.linkedin.com/in/salim-may-456a271a3/" icon={<Linkedin />} label="LinkedIn" />
            <SocialLink href="https://github.com/salimmay" icon={<Github />} label="GitHub" />
          </div>
